  ...
  <div class="content-wrapper">
          ...
          <table class="table table-striped projects">
              ...
              <tbody>
                @forelse ($blog as $blogs)
                  ...
                  <tr>
                    <th>
                      <input type="checkbox" class="selected_customer" name="selected_customers[]" value="{{ $blogs->seo_url }}" baslik="{{ $blogs->baslik }}"></th>
                      <td>
                        <p>{{ $blogs->baslik }}</p>
                      </td>
                      ...
                      <td>
                        @if(!empty($blogs->image))
                          <img alt="Avatar" class="page_image" src="/public/storage/{{ $blogs->image }}">
                        @endif
                      </td>
                      <td>
                        @if($blogs->aktif == "on")
                          <p class="d-flex align-items-center" style="color: green;">Yayında &nbsp;<i class="fa fa-circle"></i></p>
                        @else
                          <p class="d-flex align-items-center" style="color:brown;">Pasif &nbsp;<i class="fa fa-circle"></i></p>
                        @endif
                      </td>
                      <td class="project-actions text-right">
                        {{-- <a class="btn btn-primary btn-sm" href="/{!!Request::path()!!}/detay/{{ $blogs->seo_url }}">
                            <i class="fas fa-eye"></i>
                        </a> --}}
                        <a class="btn btn-info btn-sm" href="/{!!Request::path()!!}/duzenle/{{ $blogs->seo_url }}">
                            <i class="fas fa-pencil-alt">
                            </i>
                            
                        </a>
                        <a data-toggle="modal" data-target="#deleteModal" class="btn btn-danger btn-sm" href="#" onclick="destroy('{{ $blogs->seo_url }}')">
                  ...
                  @empty
                  <tr><td colspan="4"><h4 class="text-center">Blog Bulunamadı</h4></td> </tr>
                  @endforelse
          ...
          <div class="ml-3 mt-3" >
            <span class="float-right">
              <nav class="float-right mr-3">
                  <ul class="pagination">
                      <li class="page-item {{ $blog->previousPageUrl() ? '' : 'disabled' }}" aria-disabled="{{ $blog->previousPageUrl() ? 'false' : 'true' }}" aria-label="« Previous">
                          @if ($blog->previousPageUrl())
                              <a class="page-link" href="{{ $blog->previousPageUrl() }}" aria-hidden="true">Önceki</a>
                          @else
                              <span class="page-link" aria-hidden="true">Önceki</span>
                          @endif
                      </li>
                      @foreach (range(1, $blog->lastPage()) as $page)
                          <li class="page-item {{ $page == $blog->currentPage() ? 'active' : '' }}" aria-current="{{ $page == $blog->currentPage() ? 'page' : '' }}">
                              <a class="page-link" href="{{ $blog->url($page) }}">{{ $page }}</a>
                          </li>
                      @endforeach
                      <li class="page-item {{ $blog->nextPageUrl() ? '' : 'disabled' }}">
                          <a class="page-link" href="{{ $blog->nextPageUrl() }}" rel="next" aria-label="Next »">Sonraki</a>
                      </li>
                  </ul>
              </nav>
            </span>
    ...
    <script>
     ...
     function multi_delete(){
      ...
      // Seçilen Bloglari form içinde göster
      var selectedProductsText = selectedProducts.join("<br>");
      document.getElementById('selectedProductsDisplay').innerHTML = "<p>Seçilen Bloglar:</p>" + selectedProductsText;
